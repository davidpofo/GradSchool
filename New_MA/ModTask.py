...
expName = 'ModTask'  # from the Builder filename that created this script
expInfo = {'session': '', 'participant': ''}
...
PracticeBlock = data.TrialHandler(nReps=1, method='fullRandom', 
    ...
    trialList=data.importConditions('Mod practice problems.xlsx'),
...
TestBlock = data.TrialHandler(nReps=1, method='fullRandom', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('Mod task problem bank.xlsx'),
...
if thisTestBlock != None:
    for paramName in thisTestBlock:
        exec('{} = thisTestBlock[paramName]'.format(paramName))
