...
cont.boot<- mediate(MediatorModel, OutcomeModel,  boot=TRUE, sims=10000, treat="Approx.Acc.", mediator="VSWM")# 10,000 simulations of Nonparametric Boostrapped Confidence Intervals
summary(cont.boot)
```
...
library(psych)
set.seed(2018)#If the seed isn't set each time mediate is run the results will be slightly different each run because of Monte Carlo errors. 
m.1 <- mediate(Mod.Acc. ~ Approx.Acc. + (VSWM), data=MasterSheet,n.iter =10000,std = FALSE)
...
m.out <- mediate(MediatorModel_Comp, OutcomeModel, treat="Approx.Acc.", mediator=c("prop.score","Shape"), boot=TRUE, sims=1000)
summary(m.out)

plot(contcont.boot, treatment = NULL, labels = NULL,
    ...
    col = "black")
m.2 <- mediate(Mod.Acc. ~ Approx.Acc. + (Shape) + (prop.score), data=MasterSheet,n.iter =10000,std = FALSE)
...
corrgram(MasterSheet[myvars], labels=c("Approximate Arithmetic Accuracy", "Modular Arithmetic Accuracy", "Visuospatial WM Accuracy"),lower.panel=panel.pts, upper.panel=panel.conf,
...
set.seed(2018)
m.102 <- mediate(Mod.Acc. ~ Approx.Acc. + (VSWM), data=MasterSheet2,n.iter =10000,std = FALSE)# 10,000 simulations of Nonparametric Boostrapped Confidence Intervals for the first 102 subjects
