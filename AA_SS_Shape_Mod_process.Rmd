...
res<-medci(mu.x=a, mu.y=b, se.x=se.a, se.y=se.b, rho=0, alpha=.05,
      type="prodclin", plot=TRUE, plotCI=TRUE)#essentially the same as nonparametric boostrap confidence intervals because all the predictors are continuous 
```
...
library(psych)
# Analyses 
#m.1<-mediate(y="MA.Accuracy",x="AA.Accuracy",m="VSWM",data=masterappend,n.iter = 10000,std = FALSE)
#mediate.diagram(m.1,main = "Mediation Model with VSWM Composite",r.type="residual", labels("1"))
```
...
library(yhat)
all.subsets.reg<-aps(masterappend,"MA.Accuracy",list("AA.Accuracy","VSWM")) #DV and list of IVs
